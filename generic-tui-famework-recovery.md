...
- `TextStream` now exposes generic `append(text, annotations?)`, generic snapshots, Markdown presentation options, caller-controlled insets, and caller-selected `TextStreamPacing` (`tickIntervalMs`, spring, and unit-rate bounds). The Iyon assistant stream is an application wrapper over that API and requests a modestly faster 40-units/second minimum; the native `Smooth` scheduler remains responsible for every tick and render.
