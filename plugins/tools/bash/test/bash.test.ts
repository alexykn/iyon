...
describe("bash tool", () => {
  test("compiles lifecycle and multiline result fixtures through native views", () => {
    for (const state of ["preparing", "prepared", "running"] as const) {
      expect(materializeView(bashTool.renderCall({ id: "call" as never, name: "bash", arguments: { command: "echo hi" }, state }) as never)).toBeDefined();
    }
    expect(materializeView(bashTool.renderResult({ content: [{ type: "text", text: "one\ntwo" }], details: {}, isError: false }) as never)).toBeDefined();
    expect(materializeView(bashTool.renderResult({ content: [{ type: "text", text: "failed\nagain" }], details: { fullOutputPath: "/tmp/output" }, isError: true }) as never)).toBeDefined();
  });
