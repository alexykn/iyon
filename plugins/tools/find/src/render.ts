...
export function renderFindCall(call: ToolCall<{ pattern: string; path?: string }>): View {
  return toolCallLine(call.arguments === undefined ? `find — ${statusLabel(call.state)}` : `find ${call.arguments.pattern} in ${call.arguments.path ?? "."} — ${statusLabel(call.state)}`, call.state, call.pulse);
}

export function renderFindResult(result: ToolResult): View {
  const style = resultStyle(result.isError);
  return View.vertical([toolResultLine(result.isError ? "find failed" : "find result", style), toolResultLine(resultText(result), style)]).fillWidth();
}
