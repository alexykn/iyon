...
describe("app interactions", () => {
  test("handles Ctrl+C precedence and Escape", async () => {
    ...
    state = { ...result.state, activeTurn: true }; result = await handleIyonAction(state, { type: "escape" }, ctx); expect(result.state.activeTurn).toBe(false); expect(calls).toEqual(["cancel", "agent-cancel"]);
    state = { ...createInitialState({ provider: "mock", modelId: "mock" }), activeTurn: true, composerText: "steer" }; result = await handleIyonAction(state, { type: "ctrlC" }, ctx); expect(result.state.activeTurn).toBe(false); expect(result.exited).toBe(false); expect(calls).toEqual(["cancel", "agent-cancel", "cancel", "agent-cancel"]);
    state = { ...state, activeTurn: false, composerText: "" }; result = await handleIyonAction(state, { type: "ctrlC" }, ctx); expect(result.exited).toBe(true); expect(calls.at(-1)).toBe("exit");
  });
