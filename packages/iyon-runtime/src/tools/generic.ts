import { Style, View, themeColor } from "@iyon/tui";
...
export function renderGenericCall(call: ToolCall): View {
  ...
  return View.hanging(View.text("● ").style(call.pulse ? style.dim() : style).noWrap(), View.text("  ").noWrap(), View.text(`${call.name || "tool"} — ${label}`).style(style).fillWidth()).fillWidth();
}
...
export function renderGenericResult(result: ToolResult): View {
  ...
  const style = Style.new().foreground(themeColor(result.isError ? "text.error" : "text.muted"));
  ...
  return View.vertical([View.text(`${result.toolName ?? "tool"} ${title}`).style(style).fillWidth(), body]).fillWidth();
}
