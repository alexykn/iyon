...
describe("Scene extensions", () => {
  test("composes in order and supports full replacement", async () => {
    ...
    extensions.compose({ id: "one", order: 2, compose: (scene) => new Scene(scene.body, scene.history) });
    extensions.compose({ id: "two", order: 1, compose: (scene) => new Scene(scene.body, scene.history) });
