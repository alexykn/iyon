...
Tranche 11 (`§22 PERF-11.9`) is implemented in commits `0771428` and `7894c55` (`perf(tui): specialize Bun 1.4 string and style calls`, followed by the fused text-layout recipe routing fix). The canonical ABI now contains 46 functions and adds `StyleAtomRef`/`StyleRef` ranges, cstring style-atom creation, fixed scalar style creation, one-span length-preserving UTF-8 buffer creation, and cstring text constructors for one through four spans. All generated Rust/C/TypeScript bindings, manifests, layout tests, benchmark registry, and ABI reference are regenerated from the schema.
...
The clean Bun 1.4 string/style benchmark is `packages/iyon-runtime/bench/PERF-11.9-native-strings.json`, captured at source revision `7894c559426773a8cb1542164594989fae42e0f4` with `git_dirty: false`, Bun revision `34cbb9a40b4bd1bd767d134a7065e66c2432a676`, native artifact SHA-256 `448c1b1150860dc09a75fa7298afd2343b83440db7eb41ee1af78631799952e9`, schema BLAKE3 `2e4b86b766ff8db783e86905756b496e09f137e29bb0b36dc54dfa3834ebf809`, and generator BLAKE3 `a7337dcff858f38ae2b177f87758637fc2fcf2bb1d0ec1b1286722b66d00f0529`. With 2 warmups, 1,000 iterations, and 3 repeats, native versus V4 median nanoseconds were: short ASCII `2,210.917` vs `4,176.292`; short Unicode `1,969.291` vs `3,758.875`; embedded NUL `2,449.833` vs `3,173.166`; long text `2,716.792` vs `4,104.166`; styled chain `5,891.5` vs `7,804`; and one-to-four spans `2,200.5/2,669.125/2,693.625/2,831.542` vs `3,631.125/5,520.792/4,746.667/5,452.708`. Structural encoding, command words, path arrays, and NodeId arrays are zero for the native route. The benchmark includes FFI, native text/style construction, publication, and host installation; cstring transcoding is part of the measured total and is not claimed zero-cost.
