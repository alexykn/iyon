...
Tranche 10 (`§22 PERF-11.8`) is implemented in commits `2b5632c5534139d2ce347fb9afee4bde4978d3e6` and `032f07cf2c90e12ef8ee18c3df1c63bcad532b91` (`perf(tui): construct new View graphs through native builders`, followed by the pending-recipe isolation fix). The generated ABI now contains 39 functions and adds a disjoint `BuilderRef` range (`0x7ffe0001..0x7fff0001`) alongside the existing ViewRef, PathRef, and EditTxnRef ranges.

The native implementation adds an environment-owned `AxisBuilder` store. `axis_builder_begin`, typed `axis_builder_push`, `axis_builder_finish`, and `axis_builder_abort` retain only validated `(track_word, View)` values, enforce the configured 524,288-child bound, clear staged builders during host disposal, and finish through `View::native_axis_from_children`, preserving immutable `PersistentSeq` construction. Small-arity generated row and column constructors cover arities 0 through 4 and pass only scalar NodeId, gap, track, and child NativeRef arguments. Track words preserve normal, fixed, flex, flex-max, and content-max callback-builder semantics.
...
The end-to-end test is `packages/iyon-runtime/tests/tui_native_builder.test.ts`; native coverage is in `crates/iyon-native/src/tui/view_abi.rs` and generated ABI delegation/layout coverage is regenerated from the canonical schema. The clean Bun 1.4 benchmark is `packages/iyon-runtime/bench/PERF-11.8-native-builders.json`, captured at source revision `032f07cf2c90e12ef8ee18c3df1c63bcad532b91` with `git_dirty: false`, Bun revision `34cbb9a40b4bd1bd767d134a7065e66c2432a676`, native artifact SHA-256 `1eddb258ef473de9b74a571d70773d863475715738075e4b430df31ad30f28a2`, schema BLAKE3 `823264c7f1539c872782879f296f3782e157960ece5969f64007bb7e5430d801`, and generator BLAKE3 `9c69e5f6b013b2655aa249b00601622b4d569cb6806fb25863e0d71fe93f53de`. With cold graphs of 20, 200, 2,000, and 10,000 spacer children, the generated route measured `36,175 ns`, `111,725 ns`, `1,058,652.7 ns`, and `5,594,959 ns`; the pre-materialized V4 control measured `49,662.5 ns`, `212,225 ns`, `2,716,555.7 ns`, and `12,648,458 ns`. The benchmark uses 2 warmups, 3 repeats, and an iteration policy of 10/3/1 for the respective size bands. These are full route timings including one host install; they are not phase-subtracted claims.
