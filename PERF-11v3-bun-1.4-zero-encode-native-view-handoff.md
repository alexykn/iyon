...
Tranche 11 (`§22 PERF-11.9`) is implemented in commits `0771428`, `7894c55`, `c8576b7`, and `a0ecfde` (`perf(tui): specialize Bun 1.4 string and style calls`, followed by fused text-layout recipe routing, native cstring bounds, and the widened UTF-8 input bound). The canonical ABI now contains 46 functions and adds `StyleAtomRef`/`StyleRef` ranges, cstring style-atom creation, fixed scalar style creation, one-span length-preserving UTF-8 buffer creation, and cstring text constructors for one through four spans. All generated Rust/C/TypeScript bindings, manifests, layout tests, benchmark registry, and ABI reference are regenerated from the schema.
...
The retained TUI backing now fuses text layout plus simple text style/color modifiers into one compact pending text recipe. The qualified route sends only cached NodeId halves, wrap/alignment, style refs, and either a Bun 1.4 `cstring` or the same reusable UTF-8 scratch TypedArray as both `buffer` and `buffer_length`; it performs one host installation and emits no structural transport. One-to-four cstring span arities are scalar generated calls. Unsupported style dimensions, more than four spans, NUL-containing multi-span values, invalid UTF-8, and unsupported runtime/ABI conditions return to the authoritative direct/V4 path without changing host-mutation semantics.

`crates/iyon-tui` now uses compact inline storage for UTF-8 strings up to 12 bytes and owned storage for longer values. The legacy shared-page `Arc<dyn SharedUtf8Source>` remains isolated behind the existing FastShared compatibility feature; direct Native Shadow text never retains a writable JS alias or a trait object per span. Existing `TextSpan::text_mut` semantics are preserved by materializing inline/shared values into owned mutable storage on demand.

The clean Bun 1.4 string/style benchmark is `packages/iyon-runtime/bench/PERF-11.9-native-strings.json`, captured at source revision `a0ecfde1ff2105541f6f99d09821abb247ad7f0f` with `git_dirty: false`, Bun revision `34cbb9a40b4bd1bd767d134a7065e66c2432a676`, native artifact SHA-256 `9f09911b617b07658f4131b2fcff6ab7362591518fc99b84eaf55dbd2a65e463`, schema BLAKE3 `0e958fc44a24679d0503ae6606bcf1529e0b7caf5eed48097ddbe2ade7d60ef6`, and generator BLAKE3 `20435cb0e211e543dd671e6c86669cf3f205c8e77c5070f47f4d181a4a9d3c71`. With 2 warmups, 1,000 iterations, and 3 repeats, native versus V4 median nanoseconds were: short ASCII `2,953.667` vs `4,807.834`; short Unicode `2,050.916` vs `3,868.5`; embedded NUL `2,373.333` vs `3,061.208`; long text `2,574.917` vs `3,678.75`; styled chain `5,961.75` vs `7,868.875`; and one-to-four spans `2,129.25/3,286.375/2,602.791/2,775.875` vs `3,405.291/5,287.709/4,390.209/5,160.5`. Structural encoding, command words, path arrays, and NodeId arrays are zero for the native route. The benchmark includes FFI, native text/style construction, publication, and host installation; cstring transcoding is part of the measured total and is not claimed zero-cost.

Verification includes `bun run check:tui-abi`, Bun typecheck, Rust formatting, native unit and generated ABI tests, the full generated text/headless style parity tests, and the new `tui_native_strings.test.ts` Unicode/embedded-NUL/style-atom differential coverage. Production fallback remains unchanged for unsupported string/style shapes.
