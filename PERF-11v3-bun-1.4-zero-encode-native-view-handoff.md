...
Tranche 11 (`§22 PERF-11.9`) is implemented in commits `0771428`, `7894c55`, `c8576b7`, `a0ecfde`, `7b95014`, and `e438375` (`perf(tui): specialize Bun 1.4 string and style calls`, followed by fused text-layout recipe routing, native cstring bounds, widened UTF-8 input bounds, bounded UTF-8 span variants, and multi-span benchmark coverage). The canonical ABI now contains 49 functions and adds `StyleAtomRef`/`StyleRef` ranges, cstring style-atom creation, fixed scalar style creation, length-preserving UTF-8 buffer creation for one through four spans, and cstring text constructors for one through four spans. All generated Rust/C/TypeScript bindings, manifests, layout tests, benchmark registry, and ABI reference are regenerated from the schema.
...
The clean Bun 1.4 string/style benchmark is `packages/iyon-runtime/bench/PERF-11.9-native-strings.json`, captured at source revision `e43837509e3f2d2472a93604478ff8831d9b007d` with `git_dirty: false`, Bun revision `34cbb9a40b4bd1bd767d134a7065e66c2432a676`, native artifact SHA-256 `7a1dd3110fad5e4d686e1661390415c097a9d551598bce4e581753d9039f99a4`, schema BLAKE3 `f7b30e32493e2e95f86541401308e5db64103bd8a7e694cbecbfe851040025d3`, and generator BLAKE3 `20435cb0e211e543dd671e6c86669cf3f205c8e77c5070f47f4d181a4a9d3c71`. With 2 warmups, 1,000 iterations, and 3 repeats, native versus V4 median nanoseconds were: short ASCII `2,130.875` vs `4,454.125`; short Unicode `1,896.209` vs `3,490.583`; embedded NUL `2,329.875` vs `3,259.75`; embedded-NUL spans 2 `3,591.209` vs `5,779.166`; embedded-NUL spans 4 `3,841.084` vs `5,292.042`; long text `2,570.333` vs `3,518.125`; styled chain `4,962.292` vs `7,802.833`; and one-to-four ordinary spans `1,847.333/2,554.5/2,632.958/2,970.875` vs `3,708.459/5,080.75/4,278.416/4,868`. Structural encoding, command words, path arrays, and NodeId arrays are zero for the native route. The benchmark includes FFI, native text/style construction, publication, and host installation; cstring transcoding is part of the measured total and is not claimed zero-cost.
