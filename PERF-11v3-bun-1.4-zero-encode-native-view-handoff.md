...
Tranche 6 (`§22 PERF-11.4`) is implemented in commit `2197213` (`perf(tui): retain native edit lenses for changed View paths`). The canonical generated ABI now includes `path_root`, interned `path_child`, one general `view_text_layout_patch_path`, and fixed depth-1 through depth-4 text-layout path specializations. The schema adds the `PathRefResult` result mode and keeps `PathRef` distinct from `ViewRef` in the generated manifest, C header, Rust wrappers, Bun signatures, calls, conformance/layout tests, benchmark registry, and ABI reference.

The native environment runtime now owns a selector-only PathStore. Path nodes contain only `(parent, step kind, expected view kind, selector)` and never retain a View. PathRefs use a disjoint valid-handle range from ViewRefs, are interned by structural key, are bounded at `MAX_PATH_DEPTH = 128`, and validate the step family/expected parent kind before allocation. The TypeScript lineage sidecar stores only the previous root NodeId and a linked selector chain; it retains no previous View. One environment/session shape cache amortizes `path_child` calls across immutable lineages.
